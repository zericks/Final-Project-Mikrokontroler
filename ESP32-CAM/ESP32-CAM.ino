...
// Code untuk mikrokontroler ESP32-CAM
...
SoftwareSerial devkitSerial(3, 1); // RX, TX

// definisi nama dan pass wifi yang akan digunakan
const char* ssid = "Keluarga Cemara";
const char* password = "trioami2903";

// definisi telegram bot token yang didapat di bot father
String BOTtoken = "6835759006:AAEoBG7x2kZK9mtcTEXElkA52EUMmJqbylE";  // your Bot Token (Get from Botfather)

// definisi id dari telegram user
String CHAT_ID = "1419977381";
...
void setup() {
  ...
  devkitSerial.begin(9600);     // Serial untuk komunikasi dengan ESP32
  // Mematikan LED flash
  pinMode(4, OUTPUT);
  digitalWrite(4, 0);
  ...
  // Pengetesan awal program
  Serial.println("SEND PHOTO");
  devkitSerial.println("ESP32-CAM: Cek komunikasi ke ESP32 Devkit V1!");
  bot.sendMessage(CHAT_ID, "Testing Awal Program !");
  sendPhotoTelegram();
}

void loop() {
  // Cek apakah ada data yang diterima dari ESP32
  if (devkitSerial.available() > 0) {
    String command = devkitSerial.readStringUntil('\n');
    command.trim();
    if (command == "FOTO_TOLAK") {
      Serial.println("Menerima Perintah: FOTO_TOLAK");
      bot.sendMessage(CHAT_ID, "Seseorang mencoba mengakses pintu!");
      sendPhoto = true;  // Mengaktifkan flag untuk mengirim foto
      devkitSerial.println("ESP32-CAM: Gambar berhasil dikirim!");
    }
    else if (command == "FOTO_BELL") {
      Serial.println("Menerima Perintah: FOTO_BELL");
      sendPhoto = true;  // Mengaktifkan flag untuk mengirim foto
      bot.sendMessage(CHAT_ID, "Seseorang menekan BELL pintu !");
      devkitSerial.println("ESP32-CAM: Gambar berhasil dikirim!");
    }
    else if (command == "KIRIM_FOTO") {
      Serial.println("Menerima Perintah: KIRIM_FOTO");
      sendPhoto = true;  // Mengaktifkan flag untuk mengirim foto
      bot.sendMessage(CHAT_ID, "Memotret Kamera Depan. Foto Akan Segera Dikirim !");
      devkitSerial.println("ESP32-CAM: Gambar berhasil dikirim!");
    }
    else if (command == "CEK_WIFI") {
      if (WiFi.status() == WL_CONNECTED) {
        String wifiStatus = "Connected to: " + String(ssid) + ", IP: " + WiFi.localIP().toString();
        devkitSerial.println(wifiStatus);
      } else {
        devkitSerial.println("Wi-Fi not connected.");
      }
    }
    else if (command == "CEK_KOMUNIKASI") {
      devkitSerial.println("ESP32-CAM: Pesan ini dikirim dari ESP32-CAM");
    }
  }
